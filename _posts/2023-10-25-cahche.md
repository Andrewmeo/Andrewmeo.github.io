...
![image-20231023135904177](D:\Github\blog\_posts\image-20231023135904177.png)

启动客户端小程序，首先自动进行第一栏分类下的数据查询，服务端执行到断点。我们进行单步调试，可以看到，请求要查询的是分类id为16的菜品数据。在reids缓存中查询不到key为dish_16的数据，所以向数据库中查询分类下的菜品数据。

![image-20231023140309922](D:\Github\blog\_posts\image-20231023140309922.png)

查询到的三条菜品数据，放入list中，然后插入到redis缓存中后进行返回。

![image-20231023141026757](D:\Github\blog\_posts\image-20231023141026757.png)

打开redis客户端图像界面可以看到已经插入成功了。

![image-20231023141154194](D:\Github\blog\_posts\image-20231023141154194.png)

#### 1.4.2 测试读取缓存

依然在同一个位置打上断点，然后再次进行相同的请求。可以看到首先从缓存中查询到了该分类下的三条菜品数据，所以可以直接进行返回。

![image-20231023141558208](D:\Github\blog\_posts\image-20231023141558208.png)
